// ============================================================================
//  shapeData.js — מאגר הצורות, הצבעים והקטגוריות
//  ----------------------------------------------------------------------
//  ה-SVG-ים נלקחו מהפרויקט המקורי והוסבו למודול ES.
//  התוספת החדשה: CATEGORIES — שיוך כל מאכל לקבוצה, כדי שמצב
//  "התאימו לפי סוג המאכל" יוכל לבחור תשובה נכונה באמת.
// ============================================================================

/** שיוך כל מאכל לקטגוריה. זו הבסיס למצב ההתאמה. */
export const CATEGORIES = {
  fruit: {
    label: 'פירות',
    items: ['apple', 'banana', 'watermelon', 'grapes', 'cherry', 'strawberry',
            'pineapple', 'orange', 'lemon', 'pear', 'pomegranate'],
  },
  vegetable: {
    label: 'ירקות',
    items: ['carrot', 'corn', 'mushroom', 'eggplant', 'avocado'],
  },
  treat: {
    label: 'מאפים ומתוקים',
    items: ['donut', 'pizza', 'icecream', 'cupcake'],
  },
};

export const SHAPE_TYPES = Object.values(CATEGORIES).flatMap((c) => c.items);

/** מאכל → מזהה הקטגוריה שלו */
...
/** בונה מחרוזת SVG מוכנה להזרקה, עם הצבע שנבחר */
export function shapeSvg(type, color, size = 60) {
  const template = SHAPE_PATHS[type];
  if (!template) {
  ...
  const inner = template.split('COLOR').join(color);
